          ...
          <div className="grid-4">
            {team.map((t, i) => (
              <div key={t.id} className="card team-card">
                <div className="team-avatar">{t.name.split(' ').map((n: string) => n[0]).join('').substring(0, 2).toUpperCase()}</div>
                <h3>{t.name}</h3>
                <span className="badge badge-purple">{t.role}</span>
                <p>{t.bio}</p>
                <div className="team-social">
                  {t.linkedinUrl && <a href={t.linkedinUrl} target="_blank" rel="noopener noreferrer"><ExternalLink size={18} /></a>}
                  {t.twitterUrl && <a href={t.twitterUrl} target="_blank" rel="noopener noreferrer"><Globe size={18} /></a>}
                </div>
              </div>
            ))}
          </div>
