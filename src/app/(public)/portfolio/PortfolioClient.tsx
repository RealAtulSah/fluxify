    ...
    <>
      ...
      <section className="section reveal">
        <div className="container">
          ...
          {filtered.length === 0 ? (
            <p style={{ textAlign: 'center' }}>No projects found for this category.</p>
          ) : (
            <div className="grid-3">
              {filtered.map((p, i) => (
                <div key={p.id} className="card portfolio-card" onClick={() => setSelected(p)}>
                  ...
                  <div className="portfolio-stat">{p.resultStat}</div>
                </div>
              ))}
            </div>
          )}
      ...
      {selected && (
        <div className="modal-overlay" onClick={() => setSelected(null)}>
          <div className="modal-card" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-stat"><span>{selected.resultStat}</span></div>
          </div>
        </div>
      )}
